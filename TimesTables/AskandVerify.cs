...
{
    public static class AskandVerify
    {
        public static StringBuilder YesOrNo(string message)
        {
            ...
            do 
            {
                switch (count)
                {
                    ...
                    case 1: // if the count is 1 the get the corrective message
                        answer.Remove(0, 20); // clears the stringbuilder of text
            ...
            }while(!ValidAns.Any(s=>s.Equals(answer))); // while the answer isn't in the valid list the loop loops
            return answer;
